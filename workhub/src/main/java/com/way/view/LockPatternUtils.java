...
public class LockPatternUtils {
    ...
    private static String strLockFileName = "";
    ...
    private static class LockPatternFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            ...
            if (strLockFileName.equals(path)) {
                Log.d(TAG, "lock pattern file changed");
                Log.i("LockPatternUtils", strLockFileName + "--2---");
...
//		if (sLockPatternFilename == null) {
        ...
        strLockFileName = username + "_" + LOCK_PATTERN_FILE;
        String dataSystemDirectory = context.getFilesDir()
                .getAbsolutePath();
        Log.i("LockPatternUtils", strLockFileName + "--1---");
        sLockPatternFilename = new File(dataSystemDirectory
                , strLockFileName);
//			sLockPatternFilename = new File(dataSystemDirectory
//					, LOCK_PATTERN_FILE);
        Log.i("infos", sLockPatternFilename.getAbsolutePath() + "====");
        sHaveNonZeroPatternFile.set(sLockPatternFilename.length() > 0);
