...
/**
 * @author ChristLu create gesturepwd
 */
public class CreateGesturePasswordActivity extends BaseActivity implements
     ...
     * 重新绘制
     */
    private TextView mTvResetGesture;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mTvResetGesture = (TextView) findViewById(R.id.restart_gesture_password);
        mTvResetGesture.setOnClickListener(this);

        mTVSkip = (TextView) findViewById(R.id.tv_skip);
        mTVSkip.setOnClickListener(this);
        boolean isOpen = getIntent().getBooleanExtra("setting_key", false);
//		if (isOpen) {
...
//		}
    ...
    private void initPreviewViewsBackground() {
        for (int i = 0; i < mPreviewViews.length; i++) {
            for (int j = 0; j < mPreviewViews[i].length; j++) {
                mPreviewViews[i][j].setBackgroundResource(R.drawable.s1);
            }
        }
    }
    ...
    private void updateStage(Stage stage) {
        ...
        switch (mUiStage) {
            ...
            case NeedToConfirm:
                ...
                mTvResetGesture.setVisibility(View.VISIBLE);
                break;
            case ConfirmWrong:
                mLockPatternView.setDisplayMode(DisplayMode.Wrong);
                postClearPatternRunnable();
        ...
        initPreviewViewsBackground();// 清除小密码框手势痕迹
...
//			startActivity(new Intent(this, MainActivity.class));
                ...
                util.saveIntValue(Const.KEY_GESTURE, 2);
                ...
                finish();
                break;
            case R.id.restart_gesture_password:
                updateStage(Stage.Introduction);
                mTvResetGesture.setVisibility(View.GONE);
                break;
            default:
                break;
        }
    ...
    private void saveChosenPatternAndFinish() {
        LockPatternUtils lockUtil = new LockPatternUtils(this);
        lockUtil.saveLockPattern(mChosenPattern);
        showToast("设置成功");
        setResult(RESULT_OK);

        PersonalInfoUtil util = new PersonalInfoUtil(this);
        util.saveIntValue(Const.KEY_GESTURE, 1);
        Intent intent = new Intent(this, MainActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
